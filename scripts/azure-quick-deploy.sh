...
main() {
    ...
    print_header "Step 1: Checking Port Availability"
    print_message "Checking if port 80 is in use..." "$BLUE"
    if sudo lsof -Pi :80 -sTCP:LISTEN -t >/dev/null 2>&1; then
        print_message "Port 80 is in use (likely by Nginx - this is OK)" "$YELLOW"
        print_message "Application will use port 8080, Nginx will proxy from port 80" "$GREEN"
    fi
    ...
    print_header "Step 3: Removing All Containers"
    ...
    print_header "Step 4: Cleaning Docker Images"
    ...
    print_header "Step 6: Volume Management"
    ...
    # Step 7: Check for compose file
    print_header "Step 7: Selecting Docker Compose File"
    ...
    # Step 8: Build fresh images
    print_header "Step 8: Building Fresh Images"
    ...
    print_header "Step 9: Starting Containers"
    ...
    print_header "Step 11: Checking Status"
    ...
    print_message "  Via Nginx (port 80): http://$PUBLIC_IP/" "$YELLOW"
    print_message "  Direct (port 8080): http://$PUBLIC_IP:8080/SmartAgricultureNutrition/" "$YELLOW"
    print_message "  API: http://$PUBLIC_IP/api/v1/" "$YELLOW"
    print_message "  Swagger: http://$PUBLIC_IP/api/v1/swagger" "$YELLOW"
    echo
