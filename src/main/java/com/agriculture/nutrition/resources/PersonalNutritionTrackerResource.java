...
public class PersonalNutritionTrackerResource {
    ...
    private final NutritionTrackingService trackingService = new NutritionTrackingService();
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "Nutrition tracker deleted successfully"),
            @ApiResponse(responseCode = "404", description = "Nutrition tracker not found"),
            @ApiResponse(responseCode = "500", description = "Internal server error") })
    ...
    public List<MealSource> getMealSources(
        ...
        for (MealSource meal : meals) addMealSourceLinks(meal, uriInfo);
        return meals;
    }
    ...
    public Response addMealSource(
        ...
        URI location = uriInfo.getAbsolutePathBuilder().path("meal-sources").path(String.valueOf(created.getId())).build();
        return Response.created(location).entity(created).build();
    }
    ...
    public Response getHealthCorrelations(@PathParam("trackerId") long trackerId) {
        ...
        String analysis = String.format(
                ...
                tracker.getUserName(),
                tracker.getCurrentBMI(),
                tracker.getFarmToForkScore(),
